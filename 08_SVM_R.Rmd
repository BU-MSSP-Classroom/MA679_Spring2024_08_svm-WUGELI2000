...
Your code:
```{r,echo=TRUE}
svm.model <- svm(SURVIVED ~ FEMALE + DIABETES + TOBACCO, data = gTrain_COVID, type = 'C-classification', kernel = 'radial')
```
...
Your code:
```{r,echo=TRUE}
svm_model <- svm(SalePrice ~ `Lot Area`+Neighborhood+`Year Built`, data = ames_raw_2008)

predictions <- predict(svm_model, newdata = ames_raw_2009)
...
```{r,echo=TRUE}
set.seed(679)
tune.model=tune(svm,high_mileage~.,data=Auto,kernel='linear',ranges=list(cost=c(0.001,0.01,0.1,1,10,100,1000)))
summary(tune.model)
```
...
Your code:
```{r,echo=TRUE}
radial.tune=tune(svm, high_mileage~., data=Auto, kernel='radial', ranges=list(cost=c(0.1,1,10,100,1000),gamma=c(0.5,1,2,3,4)))

radial.tune$best.parameters
radial.tune$best.performance

poly.tune = tune(svm, high_mileage~., data=Auto, kernel='polynomial',
...
```{r,echo=TRUE}
set.seed(123)
train = sample(seq(1070), 800, replace = FALSE)
OJ.train = OJ[train, ]
OJ.test = OJ[-train, ]
```
...
Your code:
```{r,echo=TRUE}
OJ.svcfit = svm(Purchase~., data = OJ.train, kernel = "linear", cost=0.01)
summary(OJ.svcfit)
```
...
Your code:
```{r,echo=TRUE}
OJtrain.pred = predict(OJ.svcfit, OJ.train)
table(predict=OJtrain.pred, truth=OJ.train$Purchase)

OJtest.pred = predict(OJ.svcfit, OJ.test)
table(predict=OJtest.pred, truth=OJ.test$Purchase)
```
...
```{r,echo=TRUE}
set.seed(123)
OJ.tune = tune(svm, Purchase~., data = OJ.train, kernel = "linear", ranges=list(cost=c(0.01,0.1,0.5,1,10)))
summary(OJ.tune)
```
...
Your code:
```{r,echo=TRUE}
newOJtrain.pred = predict(OJ.tune$best.model, OJ.train)
table(predict=newOJtrain.pred, truth=OJ.train$Purchase)

newOJtest.pred = predict(OJ.tune$best.model, OJ.test)
table(predict=newOJtest.pred, truth=OJ.test$Purchase)
```
...
```{r,echo=TRUE}
set.seed(123)
OJ.tune.rkernel = tune(svm, Purchase~., data = OJ.train, kernel = "radial", ranges=list(cost=c(0.01,0.1,0.5,1,10)))
```

```{r}
OJkernel.train.pred = predict(OJ.tune.rkernel$best.model, OJ.train)
table(predict=OJkernel.train.pred, truth=OJ.train$Purchase)

OJkernel.test.pred = predict(OJ.tune.rkernel$best.model, OJ.test)
table(predict=OJkernel.test.pred, truth=OJ.test$Purchase)
```
...
Your code:
```{r,echo=TRUE}
OJ.tune.poly2 = tune(svm, Purchase~., data = OJ.train, kernel = "polynomial", 
                ranges=list(cost=c(0.01,0.1,0.5,1,10)), degree=2)
```

```{r}
OJpoly.train.pred = predict(OJ.tune.poly2$best.model, OJ.train)
table(predict=OJpoly.train.pred, truth=OJ.train$Purchase)

OJpoly.test.pred = predict(OJ.tune.poly2$best.model, OJ.test)
table(predict=OJpoly.test.pred, truth=OJ.test$Purchase)
```
...
Your answer:
In my opinion, radial kernel and polynomial kernel give best results.
~~~
Please write your answer in full sentences.
